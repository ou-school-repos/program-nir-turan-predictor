...
near-path sweep certifies only that the tested trees fail to beat the
path; it does not certify that no tree does. Accordingly, we reserve
nonexistence claims for proved results and state the scope of each sweep
explicitly.
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{(P)} The crossover threshold of $T(7,1,9)$ is exactly
    ...
    impossibility at $n = 5$ for the spherically symmetric family
    $T(x,y,z)$ is a polynomial identity (Proposition~\ref{prop:n5}).
    The threshold phenomenon for this specific graph was known in
    correspondence; the contribution here is the first fully written
    proof, placing it within the broader threshold landscape.
