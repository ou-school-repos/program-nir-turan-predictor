...
which maximises $\Hom(T,H)$, and which minimises it? Sidorenko~\cite{sidorenko1991}
settled the first: the star $K_{1,n-1}$ is always maximal, for every
...
\begin{description}[leftmargin=2.4em,style=nextline,font=\normalfont\bfseries]
  ...
  \item[(S) Bounded sweep.] A negative result of the form ``no member
    ...
    window''. This certifies the window, not the family, and never as a
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{(P)} The crossover threshold of $T(7,1,9)$ is exactly
    ...
    again (Theorem~\ref{thm:threshold}). This threshold was already
