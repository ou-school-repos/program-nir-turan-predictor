...
bipartite graphs on $2 \le m \le 15$ vertices, and all symmetric trees
of depth at most $5$ with fewer than $78$ vertices --- against the
near-path source trees defined
...
\begin{enumerate}[label=(\roman*)]
  \item \textbf{(E)} Verification of the path-minimizer theorem for the
    ...
    $n \le 22$ vertices. For $P_3$ the path is minimal but not unique;
    for $P_5$ the observed ties are among non-minimizing trees, and
    from $P_7$ onward the path is uniquely minimal in the tested range.
    The
  ...
  \item \textbf{(S)} All $1{,}018{,}690{,}329$ connected simple graphs on
    $m \le 11$ vertices, and all $608{,}930{,}559$ connected bipartite
    graphs on $2 \le m \le 15$ vertices, tested against the near-path family
    with $n \le 200$, $2 \le d \le 20$. No simple target below $12$ vertices
