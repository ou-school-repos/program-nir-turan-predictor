...
The strict inequality is certified algebraically by rational isolation of the
Perron root and an exact sign test for $\rho_2-1$.
...
for all sufficiently large $n$, both even and odd. An exact integer scan of the
even orders through $n=20{,}000$ finds its first even-$n$ crossover at
$n=17{,}340$ and no later sign change in that scanned interval. The same target
already has an odd crossover at $n=7$.
...
For the underlying looped target $\hat T(1,35,1,50)$, the exact
leading-coefficient test is slightly stronger at depth~4 than at depth~2:
\[
  \rho_2=0.999953714414\ldots,\qquad
  \rho_4=0.999932644624\ldots.
\]
The signs are certified algebraically by
\texttt{scripts/gadget\_scores.py}. This does not improve the perturbed
non-bipartite construction below, which is built from
$\hat T(2,34,1,48)$ rather than from the smaller looped witness.
