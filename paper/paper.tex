...
\begin{abstract}
We study the minimization of graph homomorphisms from trees to fixed target
graphs $H$, with emphasis on Leontovich graphs and path-minimizer failures.
Using quotient-matrix reductions and exact computational sweeps, we prove that
the spherically symmetric family $T(x,y,z)$ cannot cross over at $n=5$, verify
the $n=6$ residue computationally, and identify $n=7$ as the smallest
crossover threshold witnessed here. For targets with 2 or 3 automorphism
orbits, odd near-path crossover detection reduces to a finite base-case sign
check; for the 4-orbit tree $T(7,1,9)$, we prove that the threshold $n=13$
is permanent. We also show that the bipartite double cover preserves the
Leontovich property, which lets bounded sweeps rule out small looped targets,
and we use SMT search to isolate the smallest known Leontovich graphs,
including 15-vertex, 18-vertex, and 3,644-vertex examples.
\end{abstract}
...
target graphs (over 463~million bipartite graphs and 5.6~million trees),
no even-$n$ crossover was observed. Section~\ref{sec:doublecover}
...
ratios $a(N)/a(N-1)$ oscillate with period~2 because the looped target
$H_{\mathrm{ind}}$ breaks bipartiteness.
\end{remark}
...
So even-$n$ crossovers do not require loops; they only require enough
room for the dominant cancellation to persist.
...
extensive finite search: over 463~million bipartite graphs and
5.6~million trees, no even-$n$ crossover was observed. The
$n=17{,}340$ example above disproves the universal statement. The point
is scale, not a new mechanism: $B$'s secondary eigenvalue is close
enough to $\lambda_1$ that the parity protection survives until very
large $n$.

\subsection{A Perturbed Non-Bipartite Certificate}

Adding a single edge inside one side of a bipartite strongly Leontovich
graph produces a simple non-bipartite target. We use the looped
...
margin $2.87 \times 10^{-4}$.

Adding an edge $e$ between two sibling terminal leaves of $B'$ gives the
6,806-vertex graph $B' + e$. The script
\texttt{scripts/verify\_perturbed\_cert.py} computes its coarsest
equitable partition and obtains an 18-cell quotient matrix. On this
...
To certify the bound, \texttt{scripts/strong\_coeff.py}
...
both at 50 digits. Propagating these bounds gives a rigorous upper
bound $\rho_{\mathrm{hi}} \ge \rho_2(B'+e)$. Numerically,
...
margin, so $\rho_{\mathrm{hi}} < 1$ is a proof rather than a bare
floating-point comparison. This is certified numerical evidence (N) for
a simple non-bipartite strongly-Leontovich candidate in the
leading-coefficient sense.
...
(Proposition~\ref{prop:n5}) and the verified $n=6$ residue establish
...
\begin{enumerate}
  \item \textbf{Near-path sufficiency.} Every known Leontovich graph is
    ...
    the near-path sufficiency conjecture from Section~\ref{sec:prelim}
    holds, the bounded sweeps in this paper upgrade directly to
  ...
  \item \textbf{Minimal simple targets.} The bipartite double cover of
    ...
    (Section~\ref{sec:doublecover}) gives certified numerical evidence
    for a simple non-bipartite candidate on 6,806 vertices. Closing the
    gap to the 12-vertex lower bound from the simple-graph sweep
...
repository. The analytic theorems \textbf{(P)} are currently undergoing
formalization in Lean~4. Both the executable witness verifiers and the
formal theorem scaffolding are maintained at
