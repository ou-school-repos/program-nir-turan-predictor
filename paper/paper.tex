...
We also show that the bipartite double cover preserves the Leontovich
property, which lets bounded sweeps rule out small looped targets within the tested ranges. The
15-vertex graph $H^*$ is found by exhaustive bipartite sweep, the
18-vertex graph $H_{18}$ by SMT search, and the 3,644-vertex graph by
the double-cover construction, whose even crossover threshold is
$n = 17{,}340$.
\end{abstract}
...
target graphs (the 608,930,559 bipartite graphs on $m \le 15$
vertices), no even-$n$ crossover was observed. Section~\ref{sec:doublecover}
...
$m \le 15$ vertices, together with the 5.6~million source trees on
$n \le 22$ vertices, showed no even-$n$ crossover. The
...
both at 50 digits, yielding a rigorous upper bound $\rho_{\mathrm{hi}} \ge
\rho_2(B'+e)$. Numerically,
...
margin, so the numerical estimate stays below $1$. This is
...
repository at commit \texttt{9d538804240faf0a11241e4a8eb44e81db8bc9cb},
which is the working repository snapshot used for this paper rather
than an archival mirror. The analytic theorems \textbf{(P)} are
