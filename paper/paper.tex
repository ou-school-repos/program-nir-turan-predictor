...
no $T(x,y,z)$ admits a near-path crossover at $n=5$, record the $n=6$ case
--- which quantifies over all six trees on six vertices --- as
verified-plus-conjecture, and identify crossover threshold $n=7$ as the
smallest threshold witnessed here. For 2- or 3-orbit symmetric trees,
...
property, which transfers bounded sweep evidence to looped targets within the
tested ranges. The 15-vertex graph $H^*$ is found by exhaustive bipartite
sweep; the 18-vertex graph $H_{18}$ is isolated by a depth-2 bounded sweep
through path length $n \le 51$; and the 3,644-vertex graph from the
double-cover construction has even crossover threshold $n = 17{,}340$,
refuting the previously suggested bipartite parity hypothesis for even~$n$.
\end{abstract}
...
\begin{proof}
  ...
  For even $n$, our computations support the same ordering for this
  specific graph, but that conclusion is not proved here from parity
...
\begin{compresult}
  ...
  graph of partition $(3, m_2)$ in this completed depth-2 bounded sweep
  through path length $n \le 51$. It achieves a crossover against
  $E_n^{(2)}$ at $n = 17$
...
this partition family in the completed depth-2 sweep through path length
$n \le 51$}.

\subsubsection{\texorpdfstring{Topological Characteristics and Step-by-Step Walk Iteration of $H_{18}$}{Topological Characteristics and Step-by-Step Walk Iteration of H18}}
...
\begin{remark}[Rigorous Resolution of Other Bipartite Families]
  ...
  $n \le 51$, while the depth-dependent graph $H^*$ remains the smaller
  bipartite Leontovich example for general $E_n^{(d)}$.
\end{remark}
...
\begin{proposition}[Dominance of $y = 1$ in the tested range]
\label{prop:y1}
For every tested 4-orbit tree $T(x,y,z)$ with $y > 1$ and
$x,z \le 200$, the contracted tree $T(x,1,z)$ has strictly fewer
vertices and achieves the Leontovich crossover at an equal or earlier
threshold~$n$. Consequently, throughout the tested range, the
...
threshold statement we rely on exact computation rather than a general
algebraic proof: for every tested $(x,z)$ pair with $x, z \le 200$,
the threshold of $T(x,1,z)$ is at most that of $T(x,y,z)$ for all
$y > 1$.
\end{proof}
...
This proposition applies to bipartite trees satisfying its hypotheses, not to
the 5-orbit looped symmetric targets in Table~\ref{tab:strong_frontier},
whose odd-$n$ windows can open and later close.
...
So even-$n$ crossovers do not require loops. What they require here is a
near-antipodal mode: the two largest eigenvalues of $B$ in magnitude are
$7.284896$ and $-7.282828$, a ratio of about $0.99972$, so the alternating
contribution that protects $P_n$ at even~$n$ decays slowly enough to survive
until $n = 17{,}340$. This is not the second positive eigenvalue
$\lambda_2 \approx 5.842228$, whose ratio $\lambda_2/\lambda_1 \approx 0.802$
makes it negligible by comparison at much smaller~$n$.
...
near-antipodal eigenvalue of magnitude $7.282828$, not the smaller positive
eigenvalue $\lambda_2$.
