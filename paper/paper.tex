...
tested ranges. The 15-vertex graph $H^*$ comes from an exhaustive bipartite
sweep, the 18-vertex graph $H_{18}$ from a completed depth-2 bounded sweep
through path length $n \le 51$, and the 3,644-vertex double-cover witness has
even crossover threshold $n = 17{,}340$, refuting the previously suggested
bipartite parity hypothesis for even~$n$.
\end{abstract}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.38\textwidth]{figures/anomaly_NCQCCA.png}%
  \hfill
  \includegraphics[width=0.58\textwidth]{figures/anomaly_ratio.png}
...
the smallest previously known depth-2 Leontovich construction in the tested
depth-2 search regime. Because the exhaustive check of all 45,838
...
counterexamples to the path minimizer conjecture. Within the completed
depth-2 $(3,m_2)$ block-pattern sweep through path length $n \le 51$,
$H_{18}$ is the first such example.
...
repository as the working snapshot used for this paper rather than an
archival mirror. The analytic theorems \textbf{(P)} are
