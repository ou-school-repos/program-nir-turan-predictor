...
$n=5$. At $n=6$, four of the six source trees are covered by proofs and the
remaining two by exact computation. Thus $n=7$ is the smallest crossover
threshold found in the families studied here. For symmetric trees with two or
three orbits, an odd near-path crossover can be decided by a single finite
sign check. For the four-orbit tree $T(7,1,9)$, we prove that its threshold
...
synthesis, followed by a depth-2 sweep through $n \le 51$. Finally, a
...
dual to trees that minimize homomorphisms into a specific non-bipartite looped target.
\end{remark}
...
\]
where $A > 0$ (since $E_n^{(d)}$ is the asymptotic minimizer at odd $n$)
