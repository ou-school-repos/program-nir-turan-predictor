...
target graphs $H$, addressing open questions surrounding Leontovich graphs and
the path minimizer conjecture. By combining exact computational sweeps with
quotient-matrix reductions, we prove the impossibility of a crossover at $n=5$
for the spherically symmetric family $T(x,y,z)$ and verify the $n=6$ residue
computationally.
Within the threshold landscape studied here, the smallest crossover threshold
witnessed is $n=7$; the threshold is attained by $T(16,1,19)$. For targets
possessing symmetric automorphism orbits, quotient-eigenbasis expansions reduce
odd near-path crossover detection to a finite base-case sign check, covering
all 2-orbit and 3-orbit symmetric trees.
For the 4-orbit tree $T(7,1,9)$, we provide the first complete written proof
that its crossover threshold at $n=13$ is permanent. Finally, we prove that the
bipartite double cover operation strictly preserves the Leontovich property,
which allows bounded computational sweeps of simple graphs to formally rule
out small looped targets. Through SMT-based search and symmetric pruning, we
identify the smallest known Leontovich graphs, including a 15-vertex bipartite
target (at depth 16), an 18-vertex target for the classical depth-2 near-path,
and a 3,644-vertex simple strongly Leontovich target.
\end{abstract}

% ════════════════════════════════════════════════════════════════════════
\input{intro_preliminaries}
...
\begin{theorem}[Single Positive Eigenvalue Obstruction]
  \label{thm:spectral}
...
\begin{corollary}[Double-cover transfer for small sweeps]
  ...
  Leontovich. In particular, the simple-graph sweep up to $11$ vertices
  transfers to looped targets on at most $5$ vertices, and the bipartite
  sweep up to $14$ vertices transfers to looped targets on at most
  $7$ vertices.
\end{corollary}
...
\begin{remark}[Parity]
  ...
  coefficient. This parity effect keeps the path from being
...
\begin{remark}[Verified-plus-conjecture $n=6$ residue]
  ...
  $\Sp(2,1,1,1)$, are supported by exact computation only. We therefore
  record the $n=6$ conclusion as verified-plus-conjecture: the proven
  cases are (P), the two stubborn trees remain (E), and the near-path
  sufficiency conjecture stays open.
\end{remark}
...
bounded filter results in the sense of Remark~\ref{prop:pf}, not
...
near-path sweeps for $m \le 11$ and the 15-vertex depth-dependent bipartite
example $H^*$---highlight the steep combinatorial complexity of the
Leontovich search space. To bypass the exponential wall of graph
enumeration, we propose and analyze a novel algebraic search framework
based on Satisfiability Modulo Theories (SMT) solvers, which we use below
to discover the 18-vertex depth-2 bipartite example $H_{18}$, a target
beyond the reach of exhaustive enumeration.
...
Because $M$ is symmetric, we can apply London's Theorem. Asymptotically, the difference between the leading coefficients
for $P_n$ and $E_n$ is proportional to $-pq(p-q)^2 \le 0$ (where $(p,q)^T$ is the principal eigenvector).
Thus the $m_1=2$ family fails both the base depth-2 test at $n=5$ and the leading-coefficient test for eventual
depth-2 dominance. We therefore use this as a two-part obstruction and leave any stronger all-threshold,
all-source-tree statement to a separate Hoffman--London verification.
\end{proof}
...
\subsubsection{Topological Characteristics and Step-by-Step Walk Iteration of $H_{18}$}
...
$n = 17$. However, at $|V| = 75$, the audited odd thresholds $n \le 31$ do
not produce a crossover.
...
spectral radius, establishing the 76-vertex graph as the smallest size not
excluded by the local audit of Section~\ref{sec:local-pruning-audit}.
...
established algebraically, without search bounds.
...
By quotient-eigenbasis expansion, the homomorphism counts for large $n$
...
\begin{remark}[Bipartite parity hypothesis]
...
5.6~million trees), no even-$n$ crossover was observed. Section~\ref{sec:doublecover}
disproves the universal statement: the bipartite double cover $B$ of
$\hat T(1,35,1,50)$ overtakes the path at the even threshold
$n = 17{,}340$, together with the mechanism by which a sufficiently massive
bipartite target can silence the parity effect described below.
...
This shows that even-$n$ crossovers do not require loops, but rather a sufficiently
massive bipartite graph capable of delaying cancellation between the dominant modes.
...
extensive finite search (over 463~million bipartite graphs and 5.6~million
trees with no even-$n$ crossover observed), that $P_n$ always minimises among
trees on $n$ vertices when $n$ is even and the target is bipartite
(Remark~\ref{rem:parity} gives the mechanism: for even $n$, $P_n$'s perfectly
balanced bipartition $(n/2,n/2)$ uniquely minimises its projection onto the
dominant eigenvector, whereas odd~$n$ shares an unbalanced bipartition with
$E_n^{(d)}$ and lets the secondary eigenvector interfere). The $n=17{,}340$
crossover above disproves this as a universal statement: $B$ is bipartite,
yet $E_n$ overtakes $P_n$ at an even threshold. The resolution is scale, not
a structural exception to the parity mechanism --- $B$'s secondary eigenvalue
$\lambda_2$ is close enough to $\lambda_1$ in magnitude that the interfering
mode does not decay away before $n=17{,}340$, letting it silence the parity
protection that holds at small $n$.
...
eigenvalue shift and a leading-coefficient margin.

To move this from a high-precision point estimate to a certified bound,
\texttt{scripts/strong\_coeff.py} (\texttt{certified\_leading\_ratio}) computes
a Bauer--Fike residual $r = B\tilde u - \tilde\lambda_1\tilde u$ for the
computed unit Perron pair $(\tilde\lambda_1,\tilde u)$ of the symmetrized
18-cell quotient $B$, and a Davis--Kahan $\sin\theta$ bound on the angle
between $\tilde u$ and the true Perron vector, both at 50 significant digits
of working precision. Propagating these two bounds through the
leading-coefficient ratio via Cauchy--Schwarz gives a rigorous upper bound
$\rho_{\mathrm{hi}} \ge \rho_2(B'+e)$. Numerically,
...
\]
The certified error is roughly 41 orders of magnitude smaller than the
margin $1-\rho_{\mathrm{hi}}$, so $\rho_{\mathrm{hi}} < 1$ is a proof rather
than a bare floating-point comparison. This certificate rests on 50-digit
arbitrary-precision arithmetic (\texttt{mpmath}) being correct to the stated
precision; it is a residual-based certificate in that sense, not a
directed-rounding interval-arithmetic certificate. The construction is
therefore certified numerical evidence, evidence tag (N), for a simple
non-bipartite strongly-Leontovich candidate in the leading-coefficient sense.

% ════════════════════════════════════════════════════════════════════════
\section{Conclusion and Open Problems}
\label{sec:conclusion}

The combination of quotient-matrix reductions and exact computational sweeps
heavily constrains the landscape of Leontovich graphs. The
impossibility at $n=5$ for the spherically symmetric family $T(x,y,z)$
(Proposition~\ref{prop:n5}) and the verified $n=6$ residue establish $n=7$
as the smallest crossover threshold witnessed in the threshold landscape
studied here.
...
\begin{enumerate}
  \item \textbf{Near-path sufficiency.} Every known Leontovich graph is
    detected by the near-path family $E_n^{(d)}$. If
    the near-path sufficiency conjecture from
    Section~\ref{sec:prelim} holds---that a target $H$ is Leontovich if
    and only if $\Hom(E_n^{(d)}, H) < \Hom(P_n, H)$ for some $n$ and $d$---then
    the bounded computational sweeps in this paper upgrade directly to
    unconditional nonexistence proofs for small targets.
  \item \textbf{Minimal simple targets.} The bipartite double cover of
    $\hat{T}(1,35,1,50)$ yields a simple strongly Leontovich graph on 3,644
    vertices, and the perturbed quotient (Section~\ref{sec:doublecover})
    provides certified numerical evidence for a simple non-bipartite candidate
    on 6,806 vertices. Closing the gap between these large analytic
    constructions and the 12-vertex lower bound established by the simple-graph
    sweep remains a significant computational challenge.
\end{enumerate}

\section*{Code and Data Availability}
The C++ search routines, exact integer arithmetic formulations, and SMT solver
scripts supporting the exact finite computations \textbf{(E)} and bounded
sweeps \textbf{(S)} in this paper are available in the project repository. The
The analytic theorems \textbf{(P)} are currently undergoing formalization in Lean~4.
Both the executable witness verifiers and the formal theorem scaffolding are
maintained at \url{https://github.com/sjjaroch/leontovich-utils}.
