...
doc: ##H Convert markdown to PDF (F=docs/file.md)
	...
	pandoc $(F) --pdf-engine=xelatex -V geometry:margin=0.4in -o $(basename $(F)).pdf
...
docs: ##H Convert all tracked markdown files to PDF
	@for f in $$(git ls-files '*.md'); do \
		...
		pandoc "$$f" --pdf-engine=xelatex -V geometry:margin=0.4in -o "$$(dirname $$f)/$$(basename $$f .md).pdf" 2>&1 | grep -v "^$$" || true; \
	done
