...
public:
    TuranOracle(uint32_t n) : num_vertices(n) {
        num_words = (n + 63) / 64;
    }
     ...
     */
    uint32_t neighborhood_intersection_size(const uint64_t* adj_u, const uint64_t* adj_v) {
        uint32_t total_count = 0;
        
        // Use AVX-512 to intersect and count bits in blocks of 512 bits
        for (uint32_t i = 0; i < num_words; i += 8) {
            __m512i vec_u = _mm512_loadu_si512((__m512i*)&adj_u[i]);
            __m512i vec_v = _mm512_loadu_si512((__m512i*)&adj_v[i]);
            __m512i intersected = _mm512_and_si512(vec_u, vec_v);
            
            // Extract and popcount (Note: _mm512_popcnt_epi64 requires AVX512VPOPCNTDQ)
            // If not available, we fall back to manual popcountll on the 8 words.
            uint64_t temp[8];
            _mm512_storeu_si512((__m512i*)temp, intersected);
            for (int k = 0; k < 8; ++k) {
                total_count += __builtin_popcountll(temp[k]);
            }
        }
        return total_count;
    }
     ...
     */
    double calculate_local_weight(uint32_t vertex_idx) {
